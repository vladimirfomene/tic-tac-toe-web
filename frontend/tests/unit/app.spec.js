  ...
  describe("render startGameDialog component", () => {
    test("should render text input for player name", () => {
      expect(wrapper.html()).toContain(
        '<div class="field"><input type="text" placeholder="Player name"></div>'
      );
    });
...
describe("GameBoard.vue", () => {
  ...
  test("should render grid", () => {
    expect(wrapper.html()).toContain(
      '<gameboardcell-stub position="1"></gameboardcell-stub>'
    );
    expect(wrapper.html()).toContain(
      '<gameboardcell-stub position="2"></gameboardcell-stub>'
    );
    expect(wrapper.html()).toContain(
      '<gameboardcell-stub position="3"></gameboardcell-stub>'
    );
    expect(wrapper.html()).toContain(
      '<gameboardcell-stub position="9"></gameboardcell-stub>'
    );
  });
...
describe("Footer.vue", () => {
  ...
  test("should render footer", () => {
    expect(wrapper.html()).toContain(
      'Made with <span class="heart-icon">♥</span> in Cameroon'
    );
  });
});
...
describe("App.vue", () => {
  ...
  test("should render components", () => {
    expect(wrapper.html()).toContain(
      '<header-stub class="header"></header-stub>'
    );
    expect(wrapper.html()).toContain(
      '<startgamedialog-stub class="dialog"></startgamedialog-stub>'
    );
    expect(wrapper.html()).toContain(
      '<display-leader-board-stub class="display-leader"></display-leader-board-stub>'
    );
    expect(wrapper.html()).toContain(
      '<gameboard-stub class="board"></gameboard-stub>'
    );
    expect(wrapper.html()).toContain(
      '<leader-board-stub class="leader-board"></leader-board-stub>'
    );
    expect(wrapper.html()).toContain(
      '<footer-stub class="footer"></footer-stub>'
    );
  });
});
